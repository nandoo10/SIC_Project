...
async def reset_network_state():
    global current_hop, server, my_nid_short, adapter_name
    print("\n[CASCADE] A resetar estado da rede...")
    current_hop = -1
    new_name = f"Node-{my_nid_short} [Hop:-1]"
    if server:
        print("[CASCADE] A desligar Downlinks (Stopping Server)...")
        server.stop()
        await asyncio.sleep(1.0)
    print(f"[CASCADE] A reiniciar servidor como: {new_name}")
...
async def main_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            # --- CORREÇÃO AQUI ---
            # Removemos a lógica de adivinhação. O connect_best_candidate devolve o Hop real.
            print("[AUTO] A tentar conectar ao melhor candidato...")
            result_hop = await my_node_client.connect_best_candidate()
            
            # Nota: 'if result_hop' falha se o Hop for 0. Usamos 'is not False'.
